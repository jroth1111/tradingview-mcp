...
export function registerPineScriptTools(ctx) {
    const { server, wsClient, restClient, ensureAuthenticated } = ctx;
    server.tool("pinescript_draft_compile", `Validate PineScript code. Returns errors with line/column locations.
...
- To see indicator values: pinescript_draft_compile_and_run`, {
    ...
    server.tool("pinescript_draft_compile_and_run", `Validate AND run PineScript indicator to see computed values.
...
ON ERROR: Returns errors with line/column - fix and call again.`, {
    ...
    server.tool("strategy_backtest", `Backtest a PineScript strategy on historical data.
...
ON ERROR: Returns errors with line/column - fix and call pinescript_draft_compile again.`, {
    ...
    server.tool("strategy_backtest_chained", `Backtest a strategy that receives signals from another indicator (Receiver Strategy pattern).
...
- in_1 (second input.source) receives indicator's plot index 1`, {
    ...
    // ============================================================
    server.tool("pinescript_create_and_save", `Save PineScript as a new script to user's TradingView account.
...
ON ERROR: Returns errors with line/column - fix and call again.`, {
    ...
    server.tool("pinescript_save_version", `Update an existing script with new code (creates new version).
...
ON ERROR: Returns errors with line/column - fix and call again.`, {
    ...
    server.tool("pinescript_library_list", `List functions available in a Pine Script library.
...
RETURNS: Function names, parameters, return types.`, {
    ...
    server.tool("pinescript_list", `List user's saved Pine Scripts.
...
- study_execute → run and get values (for indicators)`, {
    ...
    server.tool("pinescript_load", `Load a Pine Script's source code by ID.
...
//# sourceMappingURL=pinescript.tools.js.map