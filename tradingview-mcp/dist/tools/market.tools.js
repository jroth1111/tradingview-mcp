...
import { symbolSchema, symbolWithExchangeSchema, timeframeSchema, marketSchema, positiveIntSchema, candleCountSchema, normalizeTimeframe, } from "../utils/validators.js";
export function registerMarketTools(ctx) {
    const { server, wsClient, restClient, ensureAuthenticated } = ctx;
    server.tool("market_candles", "Fetch OHLCV candlestick data for a symbol. Returns the most recent 50 candles; use market_candles_deep for longer history.", {
    ...
    server.tool("market_quote", "Get a real-time quote snapshot for one or more symbols.", {
    ...
    server.tool("market_ta", "Get TradingView technical analysis summary (buy/sell/neutral ratings). Pair with market_candles for context.", {
    ...
    server.tool("symbol_search", `Search for trading symbols. Returns the top 20 matches with full EXCHANGE:SYMBOL format.
...
HOW TO TELL IF NEEDED: Symbol lacks ":" separator (AAPL needs search, NASDAQ:AAPL does not).`, {
    ...
    server.tool("market_news", "Get recent news headlines for a symbol. Use limit to control size; pair with market_ta for catalysts.", {
        symbol: symbolSchema.describe("Symbol (e.g., NASDAQ:AAPL)"),
    ...
    server.tool("market_movers", "Get top market movers (gainers, losers, or high volume stocks). Use limit to control size.", {
    ...
    server.tool("market_overview", "Get a market overview (top stocks by market cap, volume, change, price, or volatility).", {
        market: marketSchema.default("america").describe("Market (america, crypto, forex, etc.)"),
    ...
    server.tool("sector_movers", "Get top movers within a specific sector (Technology, Healthcare, Finance, etc.).", {
    ...
    server.tool("market_candles_deep", "Fetch extended historical candle data with pagination (up to 40k+ bars). Returns summary + last 20 candles only.", {
        symbol: symbolSchema.describe("Symbol (e.g., NASDAQ:AAPL)"),
    ...
    server.tool("symbol_details", "Get comprehensive symbol details panel (performance, fundamentals, options Greeks). Similar to TradingView's right-hand details panel.", {
