...
export function registerChartDataTools(ctx) {
    const { server } = ctx;
    server.tool("chart_extract_active_chart", "Extract metadata from the active TradingView chart in the browser (requires Chrome DevTools MCP). Returns a JavaScript code snippet to execute in the browser console.", {}, withErrorHandling(async () => {
  ...
  return {
    ...
    server.tool("chart_extract_series_data", "Extract historical bar data from the active TradingView chart's main series or a specific indicator (requires Chrome DevTools MCP). Returns a JavaScript code snippet to execute in the browser console.", {
...
${findSourceFn}
  ...
  return {
    ...
    server.tool("chart_list_sources", "List all data sources (indicators/studies) attached to the active TradingView chart (requires Chrome DevTools MCP). Returns a JavaScript code snippet to execute in the browser console.", {}, withErrorHandling(async () => {
        const script = `(function() {
  const widget = window._exposed_chartWidgetCollection?.activeChartWidget?.value?.();
  if (!widget) {
    return { error: "No active chart widget found. Make sure TradingView is open with a chart loaded." };
...
//# sourceMappingURL=chart-data.tools.js.map