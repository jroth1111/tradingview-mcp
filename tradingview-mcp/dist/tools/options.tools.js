...
import { expirationDateSchema, symbolWithExchangeSchema } from "../utils/validators.js";
export function registerOptionsTools(ctx) {
    const { server, restClient } = ctx;
    server.tool("options_greeks", "Get options Greeks (iv, delta, gamma, theta, vega) for a symbol. Works for optionable stocks with active options chains.", {
    ...
    server.tool("options_volatility_chart", `Get implied volatility by strike price for one expiration date (the "volatility smile").
...
To find valid expiration dates: call options_in_time_iv first — response includes available expirations.`, {
    ...
    server.tool("options_in_time_iv", `Get implied volatility term structure across expirations.
...
//# sourceMappingURL=options.tools.js.map