...
export function registerAuthTools(ctx) {
    const { server, getSessionStatus, setCredentials } = ctx;
    server.tool("auth_configure", "Configure TradingView session credentials for premium data access. Follow with auth_status to verify.", {
    ...
    server.tool("auth_status", "Check current authentication status. Use after auth_login or auth_configure.", {}, withErrorHandling(async () => {
    ...
    server.tool("auth_login", "Open browser popup for TradingView login. Use this when authentication is required.", {}, withErrorHandling(async () => {
