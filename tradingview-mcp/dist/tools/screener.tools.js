...
export function registerScreenerTools(ctx) {
    const { server, restClient } = ctx;
    server.tool("scanner_enum_ordered", "Fetch ordered enum values for scanner filters (countries, sectors, exchanges, industries, ratings, etc.) from TradingView. For cached enums, use scanner_enum_values.", {
    ...
    server.tool("scanner_manifest", "List cached scanner label-products and markets available locally (fast, no network). Use to discover labelProduct/market combos for scanner_filters.", {
    ...
    server.tool("scanner_filters", "List valid scanner filter fields from the local cache. Use pattern/fields to narrow results; use scanner_enum_values for enum items.", {
        labelProduct: z.string().default("screener-stock")
        ...
        manifestPath: z.string().optional()
            .describe("Override manifest path (default: data/scanner/manifest.json or SCANNER_CACHE_MANIFEST)"),
    }, withErrorHandling(async ({ labelProduct, market, pattern, fields, includeEnumValues, includeRaw, summary, limit, offset, manifestPath, }) => {
        try {
    ...
    server.tool("scanner_enum_values", "Fetch enum values for scanner filters from the local cache. Use scanner_filters to discover enumIds.", {
        labelProduct: z.string().default("screener-stock")
            .describe("Label product (e.g., screener-stock, screener-etf, screener-crypto-cex)"),
    ...
    server.tool("scanner_metainfo", "Fetch scanner metainfo for a market (field definitions and available columns) from TradingView. For cached fields, use scanner_filters.", {
    ...
    server.tool("scanner_request", "Raw TradingView scanner request passthrough (no validation). Prefer screener_scan; responses can be large.", {
    ...
    server.tool("screener_scan", `Find stocks matching criteria. Returns symbols + requested data columns.
...
//# sourceMappingURL=screener.tools.js.map