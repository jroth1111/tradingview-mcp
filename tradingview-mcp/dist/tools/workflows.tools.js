...
import { z } from "zod";
import { symbolSchema, timeframeSchema, positiveIntSchema, marketSchema, } from "../utils/validators.js";
/**
 * Register all workflow prompts
 * Workflows are guided prompt templates for LLM execution
 */
export function registerWorkflowPrompts(ctx) {
    const { server } = ctx;
    server.prompt("backtest-strategy", "Validate and backtest a PineScript strategy", {
    ...
    server.prompt("analyze-stock", "Full stock analysis: fundamentals + technicals + news", {
    ...
    server.prompt("screen-to-idea", "Find trading ideas via screening, then analyze top picks", {
        market: marketSchema
            .default("america")
    ...
    server.prompt("options-snapshot", "Get IV term structure and volatility smile for options", {
        symbol: symbolSchema.describe("Symbol to analyze (exchange optional)"),
        expiration: z
            .string()
            .optional()
    ...
    server.prompt("indicator-evaluate", "Search, inspect, and run a technical indicator", {
    ...
    server.prompt("indicator-to-strategy-backtest", "Turn an indicator into a backtestable strategy", {
        query: z.string().describe("Indicator search query (e.g., RSI, MACD)"),
        symbol: symbolSchema.describe("Symbol to backtest (exchange optional)"),
        timeframe: timeframeSchema.default("1D").describe("Timeframe for backtest"),
    ...
    server.prompt("pinescript-iterate", "Fast compile/run loop for developing PineScript", {
    ...
    server.prompt("backtest-closed-source", "Backtest a closed-source/private indicator using a Receiver Strategy", {
...
//# sourceMappingURL=workflows.tools.js.map