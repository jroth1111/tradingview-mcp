...
import { ENDPOINTS, VALID_TIMEFRAMES, TIMEFRAME_MAP, } from "./types.js";
...
export class TradingViewClient {
     ...
     */
    releaseConnection(endpoint = this.endpoint) {
        if (this.pool) {
                ...
                const unsubscribe = connection.subscribe((event) => {
                    try {
                        if (event.name === "timescale_update") {
                            ...
                            const seriesKey = Object.keys(sessionData).find(k => k.startsWith("sds_") && sessionData[k]?.s);
                            if (seriesKey) {
                                const series = sessionData[seriesKey];
     ...
     */
    async getQuotes(symbols, fields) {
                        ...
                        try {
                            ...
                            if (event.name === "strategy_report" || event.name === "du") {
                                ...
                                if (data && (data.net_profit !== undefined || data.strategy)) {
                                    strategyReport = data.strategy || data;
                                }
                            }
     ...
     */
    async fetchIndicatorMeta(studyId) {
                                ...
                                if (data && typeof data === "object") {
                                    Object.entries(data).forEach(([key, value]) => {
                                        if (key.startsWith("st") && value && typeof value === "object") {
                                            const studyPayload = value;
                                            if (studyPayload.st && Array.isArray(studyPayload.st)) {
                                                studyPayload.st.forEach((point) => {
                                                    const p = point;
    ...
    async runChainedBacktest(opts) {
        ...
        let indicatorInputs = { ...(opts.indicatorInputs || {}) };
    ...
    async getDeepCandles(opts) {
        ...
        while (remaining > 0) {
            ...
            if (!oldest || candles.length < amount)
                break;
            cursor = oldest;
            if (opts.delayMs) {
    ...
    isStudyNotAllowedMessage(message) {
        return message.includes("Study not allowed in this connection");
    }
    extractStudyErrorMessage(params) {
        ...
        if (typeof params[1] === "string")
            return params[1];
        const candidate = params.find((value) => value && typeof value === "object");
        if (candidate?.reason)
            return candidate.reason;
        if (candidate?.error)
            return candidate.error;
        const strings = params.filter((value) => typeof value === "string");
        const descriptive = strings.find((value) => value.includes(" "))
            || strings.find((value) => value.length > 12);
        return descriptive || "Unknown error";
    }
    async withStudyEndpointFallback(run) {
    ...
    parseStrategyReport(data) {
        return {
            netProfit: data.net_profit ?? 0,
            netProfitPercent: data.net_profit_percent ?? 0,
            grossProfit: data.gross_profit ?? 0,
            grossLoss: data.gross_loss ?? 0,
            maxDrawdown: data.max_drawdown ?? 0,
            maxDrawdownPercent: data.max_drawdown_percent ?? 0,
            ...
            avgTrade: data.avg_trade ?? 0,
            avgWinningTrade: data.avg_winning_trade ?? 0,
            avgLosingTrade: data.avg_losing_trade ?? 0,
            trades: [], // Individual trade data not currently exposed by TradingView API
        };
    }
    handleError(completed, unsubscribe, connection, reject, err) {
        if (!completed) {
            unsubscribe();
     ...
     */
    handleErrorPooled(completed, unsubscribe, reject, err) {
        if (!completed) {
            unsubscribe();
            reject(err instanceof Error ? err : new Error(String(err)));
        }
