...
async function validateSession(sessionId, sessionSign) {
    try {
    ...
    if (existsSync(CONFIG_FILE)) {
        try {
            ...
            if (existing.sessionId) {
                cli.info(`Current session: ${existing.sessionId.slice(0, 8)}...`);
...
async function manualConfigure(existing) {
...
async function quickConfigure(sessionId, sessionSign) {
    cli.info("\n🔧 Quick Configuration\n");
    // Ensure config directory exists
    if (!existsSync(CONFIG_DIR)) {
        mkdirSync(CONFIG_DIR, { recursive: true });
...
main().catch((err) => {
    if (process.argv[2]) {
        ...
        logger.error("Error starting MCP server", err instanceof Error ? err.message : String(err));
    }
    process.exit(1);
});
