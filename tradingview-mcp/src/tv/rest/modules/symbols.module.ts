...
export class SymbolsModule extends BaseModule {
  async search(query: string, filter?: string): Promise<SymbolSearchResult[]> {
    ...
    if (filter) url.searchParams.set("search_type", filter);
    ...
    const data = (await resp.json()) as { symbols?: Array<{
      exchange: string;
      prefix?: string;
      symbol: string;
      description: string;
