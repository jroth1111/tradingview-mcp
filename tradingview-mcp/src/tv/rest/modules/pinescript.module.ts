import type {
  PineScriptSource,
  PineScriptSaveResult,
  PineScriptListResult,
  PineLibraryExports,
} from '../../types.js';
import type {
  PineParseTitleResult,
  PineTranslateLightResponse,
  PineSaveNewOptions,
  PineSaveNextOptions,
  PineTranslateResult,
} from '../../types-pinescript.js';
...
export class PineScriptModule extends BaseModule {
   ...
   */
  async isAuthToWrite(scriptId: string): Promise<boolean> {
   ...
   */
  async isAuthToGet(scriptId: string, version: string = "1"): Promise<boolean> {
    if (!scriptId) throw new Error("scriptId required");
   ...
   */
  async getScriptSource(scriptId: string, version: string = "1"): Promise<PineScriptSource> {
    if (!scriptId) throw new Error("scriptId required");
    ...
    return await resp.json() as PineScriptSource;
  }
   ...
   */
  async listScripts(filter: "saved" | "published" = "saved"): Promise<PineScriptListResult> {
    ...
    return await resp.json() as PineScriptListResult;
  }
   ...
   */
  async translateLightValidate(code: string, username: string): Promise<PineTranslateLightResponse> {
    ...
    return await resp.json() as PineTranslateLightResponse;
  }
   ...
   */
  async getLibraryExports(libId: string): Promise<PineLibraryExports> {
    if (!libId) throw new Error("libId required");
    ...
    return await resp.json() as PineLibraryExports;
  }
   ...
   */
  async parseTitle(code: string, username: string): Promise<PineParseTitleResult> {
    if (!code || !username) throw new Error("code and username required");
    ...
    return await resp.json() as PineParseTitleResult;
  }
   ...
   */
  async translate(opts: TranslateOptions): Promise<PineTranslateResult> {
    if (!opts.scriptId) throw new Error("scriptId required");
    ...
    return await resp.json() as PineTranslateResult;
  }
   ...
   */
  async saveNew(opts: PineSaveNewOptions): Promise<PineScriptSaveResult> {
    ...
    return await resp.json() as PineScriptSaveResult;
  }
   ...
   * FIXED: Now uses multipart/form-data with source field (matches HAR)
   */
  async saveNext(opts: PineSaveNextOptions): Promise<PineScriptSaveResult> {
