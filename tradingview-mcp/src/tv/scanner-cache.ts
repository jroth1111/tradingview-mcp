...
import type {
  ScannerCacheManifest,
  ScannerEnumOrderedResult,
  ScannerEnumValuesResult,
  ScannerManifestSummaryResult,
  ScannerFiltersResult,
  ScannerMetainfoResult,
} from "./types.js";

const DEFAULT_MANIFEST_PATH = "data/scanner/manifest.json";

let manifestCache: ScannerCacheManifest | null = null;
let manifestPathCache: string | null = null;
const metainfoCache = new Map<string, ScannerMetainfoResult>();
const enumCache = new Map<string, ScannerEnumOrderedResult>();
...
export async function loadScannerManifest(manifestPath?: string): Promise<ScannerCacheManifest> {
  const resolved = resolveManifestPath(manifestPath);
  if (manifestCache && manifestPathCache === resolved) {
    return manifestCache;
  }

  const manifest = await loadJson<ScannerCacheManifest>(resolved);
...
function getManifestEntry(manifest: ScannerCacheManifest, labelProduct: string) {
...
async function loadMetainfo(filePath: string): Promise<ScannerMetainfoResult> {
  const cached = metainfoCache.get(filePath);
  if (cached) return cached;

  const data = await loadJson<ScannerMetainfoResult>(filePath);
  metainfoCache.set(filePath, data);
  return data;
}

async function loadEnumOrdered(filePath: string): Promise<ScannerEnumOrderedResult> {
  const cached = enumCache.get(filePath);
  if (cached) return cached;

  const data = await loadJson<ScannerEnumOrderedResult>(filePath);
...
export async function getScannerFiltersFromCache(opts: {
  ...
  const rawFields = Array.isArray((metainfo as { fields?: unknown }).fields)
    ? (metainfo as { fields: Array<Record<string, unknown>> }).fields
    : [];

  const fields: ScannerFiltersResult["fields"] = [];
  ...
  if (opts.fields?.length) {
    ...
    matchedFields = requested.map(id => fieldMap.get(id)).filter(Boolean) as typeof fields;
...
export async function getScannerEnumValuesFromCache(opts: {
  ...
  const enums: ScannerEnumOrderedResult = {};
  const counts: ScannerEnumValuesResult["counts"] = {};
