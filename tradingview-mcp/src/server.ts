...
import type { AuthResult, SessionStatus, ToolContext } from "./tools/context.js";
...
export function createServer(config: MCPConfig = {}): McpServer {
  ...
  const setCredentials = (credentials: TVCredentials) => {
  ...
  async function ensureAuthenticated(): Promise<AuthResult> {
    const validation = await validateSession(config.credentials);
    if (validation.valid) {
