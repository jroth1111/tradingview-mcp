...
import { generateSignals, runBacktest, formatBacktestResult, type BacktestBar, type BacktestConfig } from "./utils/backtest.js";

function generateMockPriceData(count: number): BacktestBar[] {
  const basePrice = 150;
  const data: BacktestBar[] = [];
...
function generateMockRSIData(bars: BacktestBar[]): Array<{ time: number; value: number | null }> {
