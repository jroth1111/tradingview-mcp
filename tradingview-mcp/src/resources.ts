...
export function registerResources(ctx: ToolContext): void {
  ...
  server.resource(
    "symbol-ta",
    new ResourceTemplate("tradingview://symbol/{symbol}/ta", { list: undefined }),
    {
      description: "Technical analysis summary. Optional query: ?timeframe=1D",
      mimeType: "application/json",
    },
    async (uri, variables) => {
      const symbolInput = getTemplateVariable(
      ...
      const summary = await restClient.ta.summary(
        symbol,
  ...
  server.resource(
    ...
    new ResourceTemplate(
      "tradingview://symbol/{symbol}/fundamentals",
      { list: undefined }
    ),
    {
    ...
    async (uri, variables) => {
      ...
      const data = await restClient.fundamentals.get(
        symbol,
  ...
  server.resource(
    "symbol-news",
    new ResourceTemplate("tradingview://symbol/{symbol}/news", { list: undefined }),
    {
    ...
    async (uri, variables) => {
      ...
      const limit = limitParam
        ? parseWithSchema(
            positiveIntSchema.max(100, "Limit cannot exceed 100"),
            Number(limitParam),
            "limit"
          )
        : 10;
