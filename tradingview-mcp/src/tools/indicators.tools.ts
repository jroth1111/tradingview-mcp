...
export function registerIndicatorTools(ctx: ToolContext): void {
  const { server, restClient, wsClient, ensureAuthenticated } = ctx;

  server.tool(
    "indicator_search",
    `Search for built-in and community indicators by name.
...
NEXT: indicator_meta → study_execute`,
  ...
  server.tool(
    "indicator_meta",
    `Get indicator's configurable inputs and plot definitions.
