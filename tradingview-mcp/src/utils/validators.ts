...
export const symbolWithExchangeSchema = symbolSchema
  .refine(
    (s) => s.includes(":"),
    { message: "Symbol must include exchange prefix (e.g., NASDAQ:AAPL)" }
  )
...
export const studyIdSchema = z
  ...
  .refine(
    (id) => {
      ...
      if (id.startsWith("STD;")) return id.length > 4;
      // Published indicators: PUB;hash
      if (id.startsWith("PUB;")) return id.length > 4;
      // User indicators: USER;hash
      if (id.startsWith("USER;")) return id.length > 5;
      // Script@tv-scripting prefix
      if (id.includes("@tv-scripting")) return true;
 ...
 */
export function extractExchange(symbol: string): { exchange?: string; ticker: string } {
