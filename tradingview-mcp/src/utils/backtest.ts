...
export function generateSignals(
  data: Array<{ time: number; value: number | null; close?: number }>,
...
export function formatBacktestResult(result: BacktestResult): string {
  ...
  if (result.trades.length > 0) {
    ...
    for (const trade of recentTrades) {
      ...
      lines.push(
        `${trade.direction.toUpperCase()}: $${trade.entryPrice.toFixed(2)} → ${trade.exitPrice !== undefined ? `$${trade.exitPrice.toFixed(2)}` : "Open"} | ` +
