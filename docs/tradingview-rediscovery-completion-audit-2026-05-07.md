...
| --- | --- | --- |
| Add reusable unknown-unknown workflow to the skill | `skills/tradingview/surface-rediscovery.md`; `skills/tradingview/reference/parallel-surface-discovery-prompt.md`; linked from `skills/tradingview/SKILL.md`; validated by `pnpm run skill:validate` | verified |
...
| Macro maps browser runtime capture | `docs/tradingview-macro-maps-browser-runtime-capture-2026-05-07.md` | verified for public default `IRYY` macro-map browser `data.tradingview.com` WebSocket session, direct public non-default `US{GDP,UR,INTR,GDG}` quote snapshots, direct public historical `ECONOMICS:USGDP` `resolve_symbol`/`create_series` frames, the macro quote field list, UI indicator switch to `Interest Rate` as `quote_add_symbols` over `ECONOMICS:*INTR`, UI date-slider sequencing through `multi-group-series-snapshoter` plus `Overlay@tv-basicstudies`, and UI country group/list behavior as local filtering over the active quote set |
...
| Seasonals / forward curve runtime capture | `docs/tradingview-seasonals-forward-curve-runtime-capture-2026-05-07.md` | verified for public AAPL seasonals route, chart WebSocket `INTERNAL:SEASONALS` / `Seasonals@tv-basicstudies` flow, decoded zipped `du` JSON schema with `performance` and `seasonals` keys, Table view as local rendering over the decoded study payload, Average/Percent display controls as local presentation controls, and year-range drag from 2021 to 2002 as `create_study` / `modify_study` config `{"years":{"start":2002,"end":2026}}` with the same compressed `du` schema; verified for public CME_MINI/NYMEX futures forward-curve routes, exact `scanner.tradingview.com/futures/scan?label-product=futures-forward-curve` request/response schema for `CME_MINI:ES` and direct replay for `NYMEX:CL`, contract-chain quote WebSocket fan-out, and the same scanner body across COMEX/NYMEX/CBOT/CME/ICEUS roots for metals, energy, rates, grains, FX, crypto futures, and softs |
...
| Macro maps | component-data shell public; decompiled ChartApi quote/series data path; browser runtime capture for default `IRYY` indicator quote snapshots over `data.tradingview.com`; direct public probes for non-default indicator quote snapshots, GDP historical series frames, and macro quote field list; UI indicator switch to `Interest Rate` replaces quote symbols with `ECONOMICS:*INTR`; UI date slider uses `multi-group-series-snapshoter` plus chart overlay studies; UI country menu exposes G20/World/North America/Europe/Middle East-Africa/Mexico-South America/Asia-Pacific counts and selecting World is local filtering with no new data frames | Worker design decision |
| Seasonals | public symbol route and browser chart WebSocket runtime captured; uses `INTERNAL:SEASONALS`, `create_series`, and `Seasonals@tv-basicstudies-*` with ticker/year config; decoded `du` zipped JSON has `performance` year/month/summary data and `seasonals` year/day-of-year price points; Table view is local rendering over decoded `performance` data; Average/Percent controls are local presentation controls in the observed no-login run; year-range drag from 2021 to 2002 re-runs `create_study` / `modify_study` with `years.start=2002,end=2026` and returns the same compressed `du` schema | broader symbol-class coverage if needed for Worker modeling |
| Forward curves | public futures routes captured; exact scanner body/response schema captured for `CME_MINI:ES`; same body shape replayed for `NYMEX:CL`; same public scanner shape verified for `COMEX:GC`, `NYMEX:NG`, `CBOT:ZB`, `CBOT:ZS`, `CME:6E`, `CME:BTC`, `ICEUS:KC`, and `ICEUS:SB`; uses futures scanner `label-product=futures-forward-curve` plus quote WebSocket fan-out over contract chains | UI interaction variants |
...
- Macro maps Worker design decision after public UI/runtime behavior is now covered.
- Forward-curve UI interaction variants; broader seasonals symbol-class coverage if needed for Worker modeling.
