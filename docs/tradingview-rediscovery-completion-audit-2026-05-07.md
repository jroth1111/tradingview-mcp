...
| Seasonals | public symbol route and browser chart WebSocket runtime captured; uses `INTERNAL:SEASONALS`, `create_series`, and `Seasonals@tv-basicstudies-*` with ticker/year config; decoded `du` zipped JSON has `performance` year/month/summary data and `seasonals` year/day-of-year price points | year-range interactions and table-view schema |
...
- Seasonals remaining year-range/table-view interaction variants; forward-curve additional roots and interaction variants.
