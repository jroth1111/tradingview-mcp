...
| Seasonals | public symbol route and browser chart WebSocket runtime captured; uses `INTERNAL:SEASONALS`, `create_series`, and `Seasonals@tv-basicstudies-*` with ticker/year config; decoded `du` zipped JSON has `performance` year/month/summary data and `seasonals` year/day-of-year price points; Table view is local rendering over decoded `performance` data | year-range and Average/Percent control interactions |
...
- Seasonals remaining year-range and Average/Percent control variants; forward-curve additional roots and interaction variants.
