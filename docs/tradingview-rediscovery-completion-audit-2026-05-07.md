...
| Forward curves | public futures routes captured; exact scanner body/response schema captured for `CME_MINI:ES`; same body shape replayed for `NYMEX:CL`; uses futures scanner `label-product=futures-forward-curve` plus quote WebSocket fan-out over contract chains | additional roots and interaction variants |
...
- Forward-curve additional roots and interaction variants; broader seasonals symbol-class coverage if needed for Worker modeling.
