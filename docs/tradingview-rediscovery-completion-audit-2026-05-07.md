...
| Seasonals / forward curve runtime capture | `docs/tradingview-seasonals-forward-curve-runtime-capture-2026-05-07.md` | verified for public AAPL seasonals route and chart WebSocket `INTERNAL:SEASONALS` / `Seasonals@tv-basicstudies` flow; verified for public CME_MINI/NYMEX futures forward-curve routes, exact `scanner.tradingview.com/futures/scan?label-product=futures-forward-curve` request/response schema for `CME_MINI:ES` and direct replay for `NYMEX:CL`, and contract-chain quote WebSocket fan-out |
...
| Seasonals | public symbol route and browser chart WebSocket runtime captured; uses `INTERNAL:SEASONALS`, `create_series`, and `Seasonals@tv-basicstudies-*` with ticker/year config | decode/reproduce `du` compressed study output, year-range interactions, table-view schema |
...
- Seasonals remaining decoded study schema and interaction variants; forward-curve additional roots and interaction variants.
