...
| Macro maps browser runtime capture | `docs/tradingview-macro-maps-browser-runtime-capture-2026-05-07.md` | verified for public default `IRYY` macro-map browser `data.tradingview.com` WebSocket session, direct public non-default `US{GDP,UR,INTR,GDG}` quote snapshots, direct public historical `ECONOMICS:USGDP` `resolve_symbol`/`create_series` frames, the macro quote field list, UI indicator switch to `Interest Rate` as `quote_add_symbols` over `ECONOMICS:*INTR`, and UI date-slider sequencing through `multi-group-series-snapshoter` plus `Overlay@tv-basicstudies`; country/list switching still open |
...
- Macro maps remaining country group switch behavior and exact country-code list changes.
