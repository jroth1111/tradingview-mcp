...
- Chrome DevTools Protocol browser capture proved runtime XHR/WebSocket behavior for `advanced-chart`, `screener`, `stock-heatmap`, `market-overview`, `timeline`, `events`, `technical-analysis`, and `symbol-info`: `widgetdata` WebSockets, public pushstream open-idle, Widget Sheriff 204 checks and 400 missing-origin validation, scanner REST and body shapes for stock screener/heatmap defaults, scanner-backend enum metadata, technical-analysis scanner fields, chart-events no-data envelope, support metadata, static conversions, logo assets, and telemetry. Follow-up direct probes proved timeline SSR init-data news rows, populated Reuters event rows, and economic-calendar related-history rows with browser-origin invocation semantics.
...
| Yield curves/macro maps/seasonals | route/chunk leads; macro maps default `IRYY` browser WebSocket quote snapshots over `data.tradingview.com`; yield curves default component-data and browser-rendered US table | absent/partial | Capture macro map indicator switch, country group, and historical slider frames; capture yield country/settings UI state and seasonals parameter exploration |
...
| Widgets/embeds | widget route patterns, docs route tree, public iframe shells, S3 embed scripts, runtime host globals, entry-bundle API leads, browser runtime XHR/WebSocket host and frame evidence, timeline SSR init-data news rows, stock screener/heatmap body shapes, technical-analysis scanner fields, Widget Sheriff validation, chart-events no-data envelope, populated Reuters events schema, economic-calendar related-history schema | absent/indirect | Capture remaining Advanced Chart socket-frame postMessage deltas, optional timeline pagination/filtering, broader preset bodies, and decide first-class Worker model vs mapping to existing primitives |
