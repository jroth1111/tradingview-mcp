...
| Yield curves/macro maps/seasonals | route/chunk leads; macro maps default `IRYY` browser WebSocket quote snapshots over `data.tradingview.com`; direct public macro-map probes for non-default indicator quote snapshots, GDP historical series frames, and macro quote field list; yield curves default component-data and browser-rendered US table | absent/partial | Capture macro map filter-only indicators, country group changes, exact country-code list changes, and historical slider UI event sequencing; capture yield country/settings UI state and seasonals parameter exploration |
