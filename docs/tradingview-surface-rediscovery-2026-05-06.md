...
- Seasonals / forward curve runtime capture: `docs/tradingview-seasonals-forward-curve-runtime-capture-2026-05-07.md`
...
| Yield curves/macro maps/seasonals | route/chunk leads; macro maps default `IRYY` browser WebSocket quote snapshots over `data.tradingview.com`; direct public macro-map probes for non-default indicator quote snapshots, GDP historical series frames, and macro quote field list; macro maps UI indicator switch and historical slider sequencing captured; yield curves default component-data and browser-rendered US table; yield `available_countries` term registry; direct public AU/DE/JP non-US yield quote snapshots; direct public AU/DE/JP 10Y daily history over chart `resolve_symbol`/`create_series`; guest Add Country promo gate and guest settings menu; bundle-derived Add Country/settings/clone/delete/storage paths; seasonals public symbol route with `INTERNAL:SEASONALS` / `Seasonals@tv-basicstudies` and decoded zipped JSON `du` schema; seasonals Table view local rendering; futures forward-curve public routes with exact futures scanner schema plus quote WebSocket contract fan-out | absent/partial | Capture macro map country group changes and exact country-code list changes; capture authenticated yield Add/settings/clone/delete persistence; probe forward-curve extra interaction variants |
