...
- Chrome DevTools Protocol browser capture proved runtime XHR/WebSocket behavior for `advanced-chart`, `screener`, `stock-heatmap`, `market-overview`, `timeline`, `events`, `technical-analysis`, and `symbol-info`: `widgetdata` WebSockets, public pushstream open-idle, Widget Sheriff 204 checks and 400 missing-origin validation, scanner REST and body shapes for stock screener/heatmap defaults, scanner-backend enum metadata, technical-analysis scanner fields, chart-events no-data envelope, support metadata, static conversions, logo assets, and telemetry. Follow-up direct probes proved populated Reuters event rows and economic-calendar related-history rows with browser-origin invocation semantics.
