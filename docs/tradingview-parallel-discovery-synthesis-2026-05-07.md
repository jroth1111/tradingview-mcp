...
- Macro maps country group switches and exact country-code list changes beyond the default `IRYY`, non-default `GDP`/`UR`/`INTR`/`GDG`, GDP historical series, UI indicator switch, UI historical slider sequencing, and quote-field protocol paths proven in `docs/tradingview-macro-maps-browser-runtime-capture-2026-05-07.md`.
